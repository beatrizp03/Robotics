...
namespace rrt_planner {
    ...
    void RRTPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS *costmap) {
        ...
        std::cout << "COSTMAP GLOBAL FRAME ID: " << costmap->getGlobalFrameID() << "\n";
    ...
    bool RRTPlannerROS::makePlan(const geometry_msgs::PoseStamped& start, 
      ...
      if( planner_->planPath() ){
        ros::Duration plan_time = ros::Time::now() - start_time;
        ROS_INFO("[METRIC] Planning successful. Time taken: %.4f seconds", plan_time.toSec());
        ...
        bool PLAN_OPTIMIZATION = false;
        bool PLAN_SMOOTHING = false;
        // PLAN SMOOTHING --------------------------------------------------
        if(PLAN_OPTIMIZATION)
        {
        	planner_->optimizePath(plan);
        }
        
        
        if(PLAN_SMOOTHING)
        {
        	planner_->smoothPath(plan);
        }
        // -----------------------------------------------------------------
        ...
        // THIS IS NOT NECESSARY, ALREADY DONE BEFORE
        rrt_tree_ = planner_->getTree();
        
        // METRIC: Calculate distance for the plan -------------------------
        int nearest_node_id = planner_->getNearestNodeId(world_goal);  // Get the nearest node ID

        // Initialize total distance
        double total_distance = 0.0;

        // Backtrack from nearest node to the start node using parent nodes
        int current_id = nearest_node_id;

        // Loop through parent nodes until you reach the start node (parent_id == -1)
        while (rrt_tree_[current_id].parent_id != -1) {
            int parent_id = rrt_tree_[current_id].parent_id;

            // Compute distance between current node and its parent node
            double distance = rrt_planner::computeDistance(rrt_tree_[current_id].pos, rrt_tree_[parent_id].pos);
        ...
        ROS_INFO("[METRIC] Total path distance from RRT: %.4f meters", total_distance);
